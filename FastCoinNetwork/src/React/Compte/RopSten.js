...
class RopSten extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      web3: window.web3,
  ...
  getInfo = async () => {
    ...
    const contrat = new web3.eth.Contract(jsonContract.abi, jsonContract.networks[3].address)
    let depot = await contrat.methods.lastDepositAmount().call({from: accounts[0]})
    depot = await web3.utils.fromWei(depot, 'ether')
    let Retrait = await contrat.methods.lastWithdrawAmount().call({from: accounts[0]})
    Retrait = await web3.utils.fromWei(Retrait, 'ether')
    const Nonce = await contrat.methods.lastDepositNonce().call({from: accounts[0]})
    this.setState ({
      deposit: depot,
      account: accounts[0],
      nonce: Nonce,
  ...
  generMsg = async () => {
    ...
    let Retrait = await Contrat.methods.lastDepositAmount().call({from: account})
    Retrait = await web3.utils.fromWei(Retrait, 'ether')
    this.setState ({
      Msg: msg,
      modal2: true,
  ...
  ouvrirModal = async () => {
    ...
    const nonc = await Contrat.methods.lastWithdrawNonce().call({from: account})
    this.setState ({
      modal: true,
      nonce2: nonc
    })
  ...
  transfert = async (msg, dRetrait, mRetrait) => {
    ...
    if (dRetrait != NonceRetrait) {
      alert("Le nonce n'est pas correct!\nVeuillez vérifier qu'il correspond à celui spécifié lors de la génération du message côté Fast Coin network.\nPour cela reconnectez vous au réseau fast Coin.")
  ...
  majNonce = async () => {
    if (window.confirm("Attention, vous êtes sur le point de modifier le numéro du dernier retrait.\nCette opération est irréversible et peut vous faire perdre les fonds retirés depuis le réseau Fast Coin.\nVous pouvez consulter l'aide accessible depuis la page d'accueil.")) {
      const {account, Contrat} = this.state
      await Contrat.methods.updateWithdrawNonce().send({from: account})
      this.actual()
    }
}
...
// setadr = async () => {
//   const {account, Contrat} = this.state
//   await Contrat.methods.setContractAddress("0x659432ab0e6324Df1dE5e1f5D2D664CECA6cEb69").send({from: account})
//   alert("ok")
// }
  ...
  render() {
    return (
        <div>
          <h5 class="text-center">Vous êtes actuellement connecté au réseau Rop Sten</h5>
            ...
            <div class="row justify-content-center mt-4">
              <label>Vous souhaitez faire un nouveau dépôt afin de créditer votre compte sur le réseau fast Coin?</label>
              </div>
              <div class="row justify-content-center">
              <label>Rien de plus simple!</label>
              </div>
              <div class="row justify-content-center mt-2">
              <ul class="list-group">
              <li>Faites un virement à l'adresse suivante: 0x1cB750245A3931A80552DAEDEDf5A32B14ad6E21</li>
